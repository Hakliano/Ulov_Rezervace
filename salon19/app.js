...
function emptySlot(text) {
  return `<div class="empty-slot">${text}</div>`;
}

function fillTextOrPlaceholder(el, value, placeholder) {
  if (!el) return;
  const v = (value || '').trim();
  el.textContent = v || placeholder;
  el.classList.toggle('is-placeholder', !v);
}

function fillLinkOrPlaceholder(el, value, hrefPrefix, placeholder) {
  if (!el) return;
  const v = (value || '').trim();
  el.textContent = v || placeholder;
  el.classList.toggle('is-placeholder', !v);
  if (v && hrefPrefix) el.href = `${hrefPrefix}${v.replace(/\s/g, '')}`;
...
function renderPersonelPublic(list) {
  ...
  if (!list.length) {
    grid.innerHTML = emptySlot('Tým je prázdný. Přidejte kadeřnici v ⚙ → Personál.');
    return;
  }
...
function renderOteviraciDoba(list) {
  ...
  if (!tbody) return;
  if (!list || !list.length) {
    tbody.innerHTML = '<tr><td colspan="2" class="is-placeholder">Pracovní dobu nastavíte u týmu v ⚙ → Personál.</td></tr>';
    return;
  }
...
function renderSalon(data) {
  ...
  fillTextOrPlaceholder(document.getElementById('salon-desc'), data.description, 'Tady bude úvod salonu. Doplníte ho v ⚙ → Základní.');
  fillTextOrPlaceholder(document.getElementById('about-text'), data.description, 'Popis, adresa i kontakt se zakládají v administraci.');
  fillTextOrPlaceholder(document.getElementById('salon-address'), data.address, 'doplníte v administraci');
  fillLinkOrPlaceholder(document.getElementById('salon-phone'), data.phone, 'tel:', 'doplníte v administraci');
  fillLinkOrPlaceholder(document.getElementById('salon-email'), data.email, 'mailto:', 'doplníte v administraci');
  ...
  if (footerHint) {
    ...
    footerHint.textContent = hasPublicContent ? '' : 'Obsah se zakládá v administraci — teď je salon čistý';
    footerHint.classList.toggle('hidden', hasPublicContent);
  }
  ...
  if (obrazky.length) {
    ...
    gallery.innerHTML = emptySlot('Galerie je prázdná. První fotky nahrajete v ⚙ → Obrázky.');
  }

  const cenik = data.cenik || [];
  document.getElementById('cenik-list').innerHTML = cenik.length
    ? cenik.map(item =>
      `<div class="price-card ${item.obrazek ? 'price-card-has-image' : ''}">
        ...
        <div class="price-copy">
          ${typeof cenikPublicPriceInner === 'function' ? cenikPublicPriceInner(item, esc) : `<span class="price-name">${esc(item.nazev)}</span><span class="price-value">${item.cena} Kč</span>`}
        </div>
      </div>`
    ).join('')
    : emptySlot('Ceník je prázdný. Služby přidáte v ⚙ → Ceník — pak půjde rezervovat.');
...
function renderDelkyEdit() {
  const root = document.getElementById('delky-edit');
  if (!root) return;
  const items = salonData?.cenik || [];
  if (!items.length) {
    root.innerHTML = '<p class="admin-hint">Zatím nemáte žádné služby. Nejdřív je přidejte v záložce Ceník.</p>';
    return;
  }
  root.innerHTML = items.map((item) => `
    <div class="delky-row" data-id="${item.id}">
      <span class="delky-name">${esc(item.nazev)}</span>
      <label class="delky-field">
        <span class="delky-label">Délka</span>
        <input type="number" class="delky-minut" min="5" max="720" step="5"
          value="${parseInt(item.delka_minut, 10) || 30}" aria-label="Délka služby ${esc(item.nazev)} v minutách">
        <span class="delky-unit">min</span>
      </label>
    </div>
  `).join('');
}

async function saveDelky() {
  const msg = document.getElementById('delky-save-msg');
  if (!msg) return;
  if (!staffToken || !isMajitel()) {
    msg.textContent = 'Pro uložení se přihlaste jako majitel / správce salonu.';
    msg.className = 'status-msg error';
    return;
  }
  const byId = {};
  document.querySelectorAll('#delky-edit .delky-row').forEach((row) => {
    const id = parseInt(row.dataset.id, 10);
    let min = parseInt(row.querySelector('.delky-minut').value, 10);
    if (!Number.isFinite(min)) min = 30;
    if (min < 5) min = 5;
    if (min > 720) min = 720;
    byId[id] = min;
  });
  const cenik = (salonData.cenik || []).map((item, i) => ({
    id: item.id,
    nazev: item.nazev,
    cena: item.cena,
...
function defaultRezervaceUrl() {
  ...
  return `https://demo${SALON_ID}.ulovklienty.cz/rezervace.html`;
}
...
document.getElementById('btn-save-delky')?.addEventListener('click', saveDelky);
