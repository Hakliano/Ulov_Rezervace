...
(function () {
  ...
  var hostInit = window.location.hostname;
  var contactEmail = hostInit.indexOf("modernik") !== -1 ? "info@modernik.cz" : "info@ulovklienty.cz";
  ...
  function lockForm() {
    ...
    var msg =
      ...
      lockHours +
      " hodin, nebo napište na " + contactEmail + ".";
    setStatus(msg);
  }
    ...
    if (isLocked()) {
      ...
      setStatus(
        "Formulář je dočasně uzavřen (ochrana proti robotům). Zkuste to za " +
          lockHours +
          " h, nebo napište na " + contactEmail + "."
      );
    }
  ...
  form.addEventListener("submit", function (e) {
    ...
    var isStaging = host.indexOf("staging") !== -1;
    var isModernik = host.indexOf("modernik") !== -1;
    var contactEmail = isModernik ? "info@modernik.cz" : "info@ulovklienty.cz";
    var apiBase = isLocal
      ? "http://" + host + ":8000/api"
      : (isStaging ? "https://api-staging.ulovklienty.cz/api" : "https://api.ulovklienty.cz/api");

    var lines = [
      "=== PODKLADY PRO WEB (Moderník) ===",
      "",
      "Barvy:",
    ...
    fetch(apiBase + "/poptavka/", {
      ...
      .catch(function () {
        setStatus("Odeslání se nezdařilo. Napište prosím na " + contactEmail + " nebo zavolejte +420 797 756 746.");
      });
  });
})();
