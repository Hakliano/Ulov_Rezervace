(function () {
  ...
  const POPTAVKA_SOURCE =
    document.getElementById("poptavka-form")?.dataset.source || "ULOV KLIENTY hub";
  const CONTACT_EMAIL = "info@ulovklienty.cz";

  const form = document.getElementById("poptavka-form");
  const msg = document.getElementById("form-msg");
  const STORAGE_PREFIX = "hubCaptcha_";
  const maxAttempts = 3;
  ...
  form?.addEventListener("submit", async (e) => {
    ...
    if (payload.balicek) zpravaParts.unshift(`Projekt: ${payload.balicek}`);
