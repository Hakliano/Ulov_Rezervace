...
async function loadInfo() {
  ...
  const groups = (typeof window.groupHulinekServices === 'function'
    ? window.groupHulinekServices(info.sluzby || [])
    : [['Služby', info.sluzby || []]]);
  $('#sluzby-list').innerHTML = groups.map(([title, rows]) => `
    <div class="sluzby-group">
      <h3>${esc(title)}</h3>
      <div class="sluzby-grid">
        ${rows.map((s) => `
          <label class="sluzba-card">
            ...
            <span class="sluzba-meta">${s.delka_minut} min</span>
          </label>
        `).join('')}
      </div>
    </div>
