...
function renderSalon(data) {
  ...
  if (aboutEl) aboutEl.textContent = data.description || '';
  ...
  document.getElementById('cenik-list').innerHTML = renderPublicServices(data.cenik || []);
...
function groupPublicServices(items) {
  if (typeof window.groupHulinekServices === 'function') {
    return window.groupHulinekServices(items);
  }
  return items?.length ? [['Služby', items]] : [];
}

function renderPublicServices(items) {
  const groups = groupPublicServices(items);
  if (!groups.length) {
    return '<p class="section-note">Seznam služeb připravujeme.</p>';
  }
  return groups.map(([title, rows]) => `
    <article class="svc-group">
      <h3>${esc(title)}</h3>
      <ul>${rows.map((item) => { const p = (typeof formatCenikCena === 'function' ? formatCenikCena(item) : ''); return `<li>${esc(item.nazev)}${p ? ` — ${esc(p)}` : ''}${item.popis ? `<small class="price-popis"> ${esc(item.popis)}</small>` : ''}</li>`; }).join('')}</ul>
    </article>
  `).join('');
}
