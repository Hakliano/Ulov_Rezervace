...
const SALON_ID = 6;
const FALLBACK_HERO = 'https://haklweb.b-cdn.net/webs/salon-6/hero/530b5dcbe4194664b181b8064a5d957d.webp';
...
function renderPersonelPublic(list) {
  ...
  grid.innerHTML = list.map((p, i) => {
    ...
    return `<article class="therapist" style="--i:${i}">
      <div class="therapist-photo">${p.fotka
        ? `<img src="${esc(p.fotka)}" alt="${esc(p.jmeno)}" loading="lazy">`
        : '<div class="therapist-photo-placeholder" aria-hidden="true"></div>'}</div>
      <div class="therapist-body">
        <h3>${esc(p.jmeno)}</h3>
        ${p.specializace ? `<p class="therapist-role">${esc(p.specializace)}</p>` : ''}
        ${p.popis ? `<p class="therapist-desc">${esc(p.popis)}</p>` : ''}
        <table class="therapist-hours"><tbody>${hours}</tbody></table>
      </div>
    </article>`;
  }).join('');
}
...
function renderSalon(data) {
  ...
  if (obrazky.length) {
    ...
    gallery.innerHTML = obrazky.map((o, i) =>
      `<figure class="gallery-item" style="--i:${i}" data-url="${esc(o.url)}">
    ...
    gallery.innerHTML = `<div class="gallery-mist" aria-hidden="true"></div><p class="gallery-empty-note">Galerie se právě plní — zatím si užijte klid prostoru.</p>`;
  }
  ...
  document.getElementById('novinky-list').innerHTML = data.novinky.map(n =>
    `<article class="pulse-item">
      ...
      ${n.obrazek ? `<figure class="pulse-media"><img src="${esc(n.obrazek)}" alt="${esc(n.nadpis)}" loading="lazy"></figure>` : ''}
    </article>`
  ).join('');
