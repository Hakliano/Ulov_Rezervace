...
/* Fraňek Autoservis — staging partner ID 18. Nezasahuje do jiných salonů. */
const SALON_ID = 18;
...
function applySalonBrand(data) {
  ...
  if (!nav) return;
  if (data.logo_url) {
    nav.innerHTML = `<img class="nav-logo-img brand-logo" src="${esc(data.logo_url)}" alt="${esc(data.name || 'Logo')}">`;
...
function renderSalon(data) {
  if (!data) return;
  if (data.name) document.title = `${data.name} · Dobřichovice`;
  ...
  setText('salon-name', data.name);
  const _nav = document.getElementById('nav-logo');
  if (_nav) _nav.dataset.fallbackText = data.name || 'Fraňek';
  applySalonBrand(data);
  setText('footer-name', data.name || 'Fraňek Autoservis');
  setText('salon-desc', data.description);
  setText('about-text', data.description);
  setText('salon-address', data.address);

  const phoneEl = document.getElementById('salon-phone');
  if (phoneEl) {
    phoneEl.textContent = data.phone || '—';
    phoneEl.href = data.phone ? `tel:${String(data.phone).replace(/\s/g, '')}` : '#';
  }
  ...
  const cenikSection = cenikList?.closest('section');
  const cenik = data.cenik || [];
  if (cenikList) {
    if (cenik.length) {
      cenikList.innerHTML = cenik.map(item =>
      ...
      cenikSection?.classList.remove('hidden');
    } else {
      cenikList.innerHTML = '';
      cenikSection?.classList.add('hidden');
    }
  }

  const novinkyList = document.getElementById('novinky-list');
  const novinkySection = novinkyList?.closest('section');
  const novinky = data.novinky || [];
  if (novinkyList) {
    if (novinky.length) {
      novinkyList.innerHTML = novinky.map(n =>
...
document.getElementById('nav-toggle')?.addEventListener('click', () => {
  /* Mobilní menu řeší app.js (Fraňek UI) — tady nic. */
});
...
/* scroll listener pro .scrolled na #nav — Fraňek header je sticky jinak; no-op pokud chybí */
...
Promise.all([fetchSalon(), fetchBunnyStatus(), fetchPersonel()])
  ...
  .catch(err => {
    ...
    if (banner) {
      banner.textContent = 'Obsah z API se nepodařilo načíst. UI zůstává dostupné.';
