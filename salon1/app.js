...
function personelEditCard(z) {
  ...
  return `<div class="personel-edit-card" data-id="${id}">
    <div class="personel-edit-head">
      <strong>${esc(z.jmeno || 'Nový èlen týmu')}</strong>
    ...
    ${id && z.aktivni === false ? '<p class="admin-hint error">Úèet deaktivován — zamìstnanec se nemùže pøihlásit. Obnovte zaškrtnutím „Aktivní“ v rezervacích › Kadeønice.</p>' : ''}
  </div>`;
}
...
async function deletePersonelCard(card) {
  const id = card.dataset.id;
  if (!id || !confirm('Deaktivovat úèet tohoto èlena týmu?\n\nÚèet se nesmaže — zùstane historie a audit. Zamìstnanec se už nepøihlásí.')) return;
...
async function uploadPersonelPhoto(e, card) {
  ...
  const id = card.dataset.id;
  if (!file || !id) {
    alert('Nejdøíve uložte èlena týmu, pak nahrajte fotku.');
    e.target.value = '';
    return;
  }
  const msg = document.getElementById('status-msg');
...
function renderSalon(data) {
  ...
  document.getElementById('nav-logo').textContent = data.name.split(' ').pop()?.toUpperCase() || data.name;
  ...
  const setContact = (addrId, phoneId, emailId) => {
    document.getElementById(addrId).textContent = data.address;
  ...
  setContact('salon-address', 'salon-phone', 'salon-email');
  setContact('salon-address-footer', 'salon-phone-footer', 'salon-email-footer');
  ...
  if (obrazky.length) {
    gallery.innerHTML = obrazky.map((o, i) =>
      `<figure class="gallery-slide" data-url="${esc(o.url)}" style="--i:${i}">
        <img src="${esc(o.url)}" alt="${esc(o.popis)}" loading="lazy">
        ${o.popis ? `<figcaption>${esc(o.popis)}</figcaption>` : ''}
      </figure>`
    ).join('');
    gallerySection?.classList.remove('hidden');
    gallery.querySelectorAll('.gallery-slide').forEach(el => {
  ...
  document.getElementById('novinky-list').innerHTML = data.novinky.map((n, i) =>
    `<article class="news-item ${n.obrazek ? 'has-img' : ''}" style="--i:${i}">
      ${n.obrazek ? `<div class="news-visual"><img src="${esc(n.obrazek)}" alt="${esc(n.nadpis)}" loading="lazy"></div>` : ''}
...
async function handleLogin(e) {
  ...
  try {
    ...
    msg.textContent = err.message || 'Nesprávné pøihlašovací jméno nebo heslo.';
    msg.className = 'status-msg error';
    console.error(err);
  }
}
...
async function handleNovinkaUpload(e) {
  ...
  if (!row.dataset.id) {
    msg.textContent = 'Nejdøív uložte novinku (tlaèítko „Uložit textová data“), pak nahrajte obrázek.';
    msg.className = 'status-msg error';
    input.value = '';
    return;
  }
...
async function handleSave() {
  const msg = document.getElementById('status-msg');
  if (!staffToken || !isMajitel()) {
    msg.textContent = 'Pro uložení se pøihlaste jako majitel salonu.';
    msg.className = 'status-msg error';
    return;
  }
...
document.getElementById('btn-add-personel').addEventListener('click', () => {
  personelAdminData.push({
    ...
    rozvrh: [0, 1, 2, 3, 4, 5, 6].map((den) => ({
      den, den_nazev: ['Pondìlí', 'Úterý', 'Støeda', 'Ètvrtek', 'Pátek', 'Sobota', 'Nedìle'][den],
